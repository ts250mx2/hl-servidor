...
export default function BarChart({ items }: BarChartProps) {
  ...
  return (
    <div ref={ref} className="chart-wrap">
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Llamadas por agente">
        ...
        {items.map((item, i) => {
          ...
          return (
            <g key={item.id} className={`chart-bar ${dim ? 'dim' : ''}`}>
              <text className="chart-label" x={x0 - 8} y={y + ROW_H / 2 + 4} textAnchor="end">
                <title>{item.label}</title>
                {truncate(item.label)}
              </text>
              <path d={barPath(x0, barY, w, BAR_H)} fill={item.color} />
              <text className="chart-label" x={x0 + w + 6} y={y + ROW_H / 2 + 4} style={{ fontWeight: 600 }}>
