...
## Latencia, tokens y gasto estimado

El proxy mide cuánto tarda cada llamada (desde que llega hasta que termina la respuesta del proveedor, streaming incluido) y lee los tokens de la respuesta sin alterarla: `usage` de Anthropic y OpenAI (chat y Responses) y `usageMetadata` de Gemini, tanto en JSON como en streams SSE. En chat/completions de OpenAI agrega `stream_options.include_usage` para que el stream traiga el uso. Todo queda en `tblBitacora` (`DuracionMs`, `TokensEntrada`, `TokensSalida`, `TokensCacheLectura`, `TokensCacheEscritura`, `CostoUsd`).

El gasto se calcula con **Catálogo → Precios**: USD por millón de tokens por proveedor y modelo. El modelo es un prefijo con límite de guion: `claude-sonnet-5` cubre `claude-sonnet-5-20260101`, pero `gpt-5` no cubre `gpt-5.6-sol` (otra versión necesita su propio precio); gana el prefijo más largo. Viene sembrado con valores orientativos de las listas públicas; verifícalos y captura los modelos que falten (una llamada de un modelo sin precio guarda tokens pero `CostoUsd` queda nulo y Estadísticas lo cuenta como "sin precio"). Los precios se cachean un minuto en el proxy y se refrescan al editarlos. Tras capturar o corregir precios, el botón **Recalcular gasto** (o `npm run db:recalcular`) vuelve a calcular el gasto de toda la bitácora.

En **Estadísticas** la métrica se elige arriba: llamadas, gasto, tokens o latencia, por periodo y por agente, aplicación, proveedor o modelo. El resumen muestra el gasto estimado de la semana y la latencia promedio del día. Migración para bases previas: `db/migracion-bitacora-uso.sql`.
